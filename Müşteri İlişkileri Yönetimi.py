from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QVBoxLayout, QHBoxLayout, QListWidget, QComboBox, QDateTimeEdit
...
class Musteri:
    def __init__(self, ad, iletisim_bilgileri):
        self.ad = ad
        self.iletişim_bilgileri = iletisim_bilgileri
...
class Arayuz(QWidget):
    ...
    def satis_ekle(self):
        ...
        musteri = self.musteriler[selected_musteri_index]  # Seçilen müşteriyi al
        satis = Satis(satis_no, [])  # Yeni bir Satış örneği oluştur
        musteri.satis_ekle(satis)  # Müşterinin satışlar listesine yeni satışı ekle

        conn = sqlite3.connect('crm.db')  # Veritabanına bağlan
        c = conn.cursor()
        c.execute("INSERT INTO satis (musteri_ad, satis_no) VALUES (?, ?)", (musteri.ad, satis_no))
        conn.commit()
        conn.close()
    ...
    def destek_talebi_olustur(self):
        ...
        musteri.destek_talebi_oluştur(destek_talebi)  # Müşterinin destek talepleri listesine yeni talebi ekle

        conn = sqlite3.connect('crm.db')  # Veritabanına bağlan
        c = conn.cursor()
        c.execute("INSERT INTO destek (musteri_ad, talep_no) VALUES (?, ?)", (musteri.ad, talep_no))
        conn.commit()
        conn.close()
